...
const REFERENCE_SOLUTIONS: Record<string, ReferenceSolution> = {
  ...
  'search-engine': {
    ...
    connections: [
      { from: 'web-client', to: 'cdn' },
      { from: 'web-client', to: 'api-gateway' },
      { from: 'api-gateway', to: 'load-balancer-l7' },
      { from: 'load-balancer-l7', to: 'app-server' },
      { from: 'app-server', to: 'redis-cache' },
      { from: 'app-server', to: 'search-engine' },
      { from: 'worker', to: 'search-engine' },
      { from: 'worker', to: 'object-storage' },
      { from: 'redis-cache', to: 'search-engine' },
      { from: 'search-engine', to: 'relational-db' },
      { from: 'logging-service', to: 'app-server' },
      { from: 'logging-service', to: 'worker' },
    ],
    keyInsights: [
      'Search Engine (Elasticsearch) stores the inverted index — maps terms to document postings lists for sub-millisecond full-text lookups across 50M+ items',
      'Worker crawls and indexes documents asynchronously — decouples write-heavy indexing from the read-heavy query path',
      'Redis Cache serves the top-1000 popular queries from memory — the same searches repeat constantly and never need to hit Elasticsearch again',
      'Object Storage holds raw document corpus and crawl data; Relational DB stores document metadata and crawl state',
      'Sharding the Elasticsearch index by document ID enables parallel scatter-gather query execution across shards',
    ],
    tradeoffs: [
      ...
      { decision: 'Real-time indexing via Worker vs batch indexing', reason: 'Worker-based real-time indexing makes new documents searchable instantly but increases write amplification; batch indexing is efficient but has a delay window' },
      { decision: 'Redis cache granularity: exact query vs normalized terms', reason: 'Caching exact query strings has low collision rate; caching by stemmed/normalized terms improves hit rate but may return slightly different rankings' },
    ],
    antiPatterns: [
      'Using LIKE %keyword% SQL queries instead of a Search Engine — O(n) full table scan per query, unusable past 100K documents',
      'Indexing documents synchronously in the API request path — blocks the App Server and degrades search latency',
      ...
      'No Redis Cache for popular queries — the same top searches re-execute expensive Elasticsearch lookups on every request',
    ],
  },
  ...
  'live-scoreboard': {
    ...
    connections: [
      { from: 'web-client', to: 'api-gateway' },
      { from: 'web-client', to: 'websocket-server' },
      { from: 'api-gateway', to: 'load-balancer-l7' },
      { from: 'load-balancer-l7', to: 'app-server' },
      { from: 'app-server', to: 'pub-sub' },
      { from: 'app-server', to: 'key-value-store' },
      { from: 'pub-sub', to: 'websocket-server' },
      { from: 'key-value-store', to: 'relational-db' },
      { from: 'logging-service', to: 'app-server' },
    ],
    keyInsights: [
      'WebSocket Server pushes score updates to 80K concurrent viewers in real time — eliminates the latency and bandwidth waste of client polling',
      'Pub/Sub broadcasts score-update events to all connected App Servers simultaneously — each server fans out to its own WebSocket connections',
      'Key-Value Store (Redis Sorted Sets) holds live scores with sub-10ms reads — the Relational DB is never polled per viewer',
      'Horizontal scaling with multiple App Servers + Load Balancer absorbs unpredictable 10x traffic spikes during finals',
    ],
    tradeoffs: [
      { decision: 'WebSocket Server vs SSE for score updates', reason: 'WebSocket enables bidirectional communication (chat, reactions); SSE is simpler for unidirectional score push but limits interactivity' },
      { decision: 'Pub/Sub fan-out vs direct server-to-server push', reason: 'Pub/Sub decouples score ingestion from delivery and scales horizontally; direct push breaks when multiple servers hold different viewer connections' },
      { decision: 'Key-Value Store (Redis) vs Relational DB for live scores', reason: 'Key-Value Store gives sub-10ms reads and supports sorted sets for leaderboards; the Relational DB is the durable source of truth only' },
    ],
    antiPatterns: [
      'Polling the Relational DB every second per viewer — 80K clients × 1 QPS = 80K DB queries/sec for data that changes only a few times per over',
      'Single App Server handling all WebSocket connections — one crash disconnects every viewer simultaneously',
      'Skipping Pub/Sub and having App Servers query the Key-Value Store on a timer — still generates unnecessary load and misses the event-driven push model',
      'Not planning for traffic spikes — a World Cup final can bring 10x normal traffic in seconds; the Load Balancer + multiple App Servers are mandatory',
    ],
  },
  ...
  'ride-hailing': {
    ...
    connections: [
      { from: 'mobile-client', to: 'api-gateway' },
      { from: 'mobile-client', to: 'websocket-server' },
      { from: 'mobile-client', to: 'cdn' },
      { from: 'api-gateway', to: 'load-balancer-l7' },
      { from: 'load-balancer-l7', to: 'app-server' },
      { from: 'app-server', to: 'geospatial-index' },
      { from: 'app-server', to: 'pub-sub' },
      { from: 'websocket-server', to: 'pub-sub' },
      { from: 'geospatial-index', to: 'relational-db' },
      { from: 'logging-service', to: 'app-server' },
    ],
    keyInsights: [
      'Geospatial Index (H3/Quadtree in Redis) powers sub-50ms "nearby drivers" queries — SQL ST_Distance scans on lat/lng cannot handle 25K concurrent matching requests',
      'Driver locations update every 3s via WebSocket and are stored in the Geospatial Index with TTL-based expiry — stale drivers are automatically removed',
      'Pub/Sub broadcasts driver location updates to riders in real time via WebSocket Server — no polling needed',
      'CDN serves map tiles at the edge — map rendering must be instant; tiles cannot flow through the App Server origin',
    ],
    tradeoffs: [
      { decision: 'Geospatial Index (H3/Redis) vs SQL ST_Distance', reason: 'Geospatial Index gives O(1) radius queries and sub-50ms matching; SQL geographic queries do full index scans and degrade past 10K concurrent requests' },
      { decision: 'WebSocket Server for real-time location vs polling', reason: 'WebSocket enables bidirectional real-time location streaming (driver→server, server→rider); polling adds latency and wastes bandwidth' },
      { decision: 'Push-based matching vs pull-based driver acceptance', reason: 'Push assigns the nearest driver immediately (fast); pull lets drivers accept/reject but adds a round-trip to every match attempt' },
    ],
    antiPatterns: [
      'Using SQL queries with ST_Distance on every ride request — full table scans cannot handle 25K concurrent matching requests at sub-155ms',
      'Not expiring stale Geospatial Index entries — showing drivers who went offline 5 minutes ago degrades rider experience and corrupts matching',
      'Serving driver location updates via REST polling instead of WebSocket — 25K riders polling every 3s = 8K requests/sec of pure overhead',
      'Serving map tiles from App Servers instead of CDN — map rendering latency on mobile devices makes the app feel broken',
    ],
  },
  ...
  'social-feed': {
    ...
    connections: [
      ...
      { from: 'load-balancer-l7', to: 'app-server' },
      { from: 'app-server', to: 'redis-cache' },
      { from: 'app-server', to: 'event-stream' },
      { from: 'worker', to: 'event-stream' },
      { from: 'worker', to: 'object-storage' },
      { from: 'redis-cache', to: 'wide-column-store' },
      { from: 'event-stream', to: 'wide-column-store' },
      { from: 'cdn', to: 'object-storage' },
      { from: 'logging-service', to: 'app-server' },
      { from: 'logging-service', to: 'worker' },
    ],
    keyInsights: [
      'Fan-out on write: when a user posts, the Event Stream (Kafka) triggers Workers that pre-write the post ID into each follower\'s timeline in the Wide-Column Store — reads are O(1) Redis lookups',
      'Redis Cache is the primary read store for timelines — 500K concurrent readers get sub-160ms responses without ever touching the Wide-Column Store directly',
      'Wide-Column Store (Cassandra) stores user timelines as ordered lists — its write-optimised wide-column model handles the massive fan-out write volume',
      'CDN + Object Storage offload all media entirely from App Servers — photos and videos never flow through the API layer',
    ],
    tradeoffs: [
      { decision: 'Fan-out on write vs fan-out on read', reason: 'Write fan-out gives O(1) feed reads but is catastrophic for celebrities (50M writes per post); hybrid strategy pushes to regular followers on write and merges celebrity posts on read' },
      { decision: 'Wide-Column Store (Cassandra) vs Relational DB for timelines', reason: 'Cassandra\'s partition-per-user model excels at write-heavy fan-out at scale; a relational DB would require expensive follower JOIN queries on every timeline read' },
      { decision: 'Event Stream (Kafka) vs direct Worker invocation', reason: 'Kafka provides durable fan-out with replay capability; direct invocation is simpler but loses fan-out jobs on Worker failure' },
    ],
    antiPatterns: [
      'Querying the Wide-Column Store directly for every timeline load — N follower queries per request at 500K concurrent readers will saturate the DB',
      'Fan-out on write for all celebrity accounts — a 50M-follower post generates 50M Cassandra writes in minutes; hybrid on-read merge for high-follower accounts is mandatory',
      'Storing media blobs in the Wide-Column Store instead of Object Storage — explodes storage costs and makes multi-resolution delivery impossible',
      'No Event Stream between the write path and fan-out Workers — a viral post triggers millions of synchronous writes that block the API response',
    ],
  },
  ...
  'video-streaming': {
    ...
    connections: [
      ...
      { from: 'app-server', to: 'event-stream' },
      { from: 'worker', to: 'event-stream' },
      { from: 'worker', to: 'transcoder' },
      { from: 'transcoder', to: 'object-storage' },
      { from: 'redis-cache', to: 'relational-db' },
      { from: 'event-stream', to: 'relational-db' },
      { from: 'cdn', to: 'object-storage' },
      { from: 'logging-service', to: 'app-server' },
      { from: 'logging-service', to: 'worker' },
    ],
    keyInsights: [
      'CDN is the backbone — video bytes must NEVER come from the origin App Server; CDN edge nodes serve 95%+ of the 500K concurrent streams',
      'Transcoder generates 480p, 720p, 1080p, 4K renditions from raw uploads and writes encoded variants to Object Storage; CDN pulls from Object Storage',
      'Event Stream decouples uploads from transcoding: Workers consume upload events and send raw video to the Transcoder — the upload API returns immediately',
      'Redis Cache stores video metadata, thumbnails, and watch progress — read millions of times per second and change rarely',
    ],
    tradeoffs: [
      { decision: 'Pre-encode all resolutions vs just-in-time Transcoder', reason: 'Pre-encoding every variant uses 6–10x Object Storage but enables instant adaptive bitrate switching; JIT saves storage but adds startup delay and Transcoder compute per stream' },
      { decision: 'CDN push warming vs pull caching', reason: 'Push pre-loads popular/new content to edge nodes before viewers request it (zero cold-start); pull caches on first request (cold-start latency for new content)' },
      { decision: 'Event Stream (Kafka) vs direct Worker invocation for transcoding', reason: 'Kafka provides durable upload events with replay on Worker failure; direct invocation is simpler but permanently loses transcoding jobs when a Worker crashes' },
    ],
    antiPatterns: [
      'Serving video bytes directly from App Servers — a single viral video at 4K can generate terabytes/sec that no origin server cluster can handle',
      'Encoding videos synchronously in the upload API path — blocks the App Server for minutes; always fan out to Worker + Transcoder via Event Stream',
      'Skipping adaptive bitrate (HLS/DASH) — forces users on slow networks to buffer constantly; encoding a single fixed resolution is unacceptable on a video platform',
      'Storing video binary files in the Relational DB — use Object Storage (S3) for all video content; the DB stores metadata only',
    ],
  },
