...
async function main() {
  ...
  // ── Achievements ──────────────────────────────────────────────────────────────────
  const achievements = [
    { slug: 'first-architecture', title: 'First Architecture', description: 'Complete your first mission',            icon: '🏗️', xpBonus: 50  },
    { slug: 'budget-master',      title: 'Budget Master',      description: 'Stay under budget by 5%',               icon: '💰', xpBonus: 25  },
    { slug: 'speed-demon',        title: 'Speed Demon',        description: 'Achieve sub-170ms latency',              icon: '⚡',  xpBonus: 35  },
    { slug: 'scale-master',       title: 'Scale Master',       description: 'Handle 10,000+ concurrent users',        icon: '📈', xpBonus: 75  },
    { slug: 'global-architect',   title: 'Global Architect',   description: 'Complete the Global Expansion mission',   icon: '🌍', xpBonus: 100 },
    { slug: 'async-master',       title: 'Async Master',       description: 'Complete the File Converter mission',     icon: '📁', xpBonus: 60  },
    { slug: 'cache-king',         title: 'Cache King',         description: 'Complete the URL Shortener mission',      icon: '⚡',  xpBonus: 70  },
    { slug: 'realtime-guru',      title: 'Realtime Guru',      description: 'Complete the Live Scoreboard mission',    icon: '🏏', xpBonus: 80  },
    { slug: 'system-design-master',title:'System Design Master',description: 'Complete all 13 missions',               icon: '🏆', xpBonus: 500 },
  ];
  ...
  await prisma.mission.upsert({
    ...
    update: { components: mvpComponents, learningPath: 'foundations', skillLevel: 'beginner' },
    create: {
      slug: 'mvp-launch', title: 'Mission 1: MVP Launch',
      difficulty: 1, estimatedTime: '15-20 min', xpReward: 150, order: 1,
      ...
      objectives: JSON.stringify(['Handle 1,000 concurrent users','Maintain 99% availability','Respond in under 200ms','Stay within $500/month budget']),
      requirements: JSON.stringify({
        traffic: { concurrent: 1000, daily: 10000 },
        performance: { latencyMs: 200, availability: 99.0 },
        budget: 500, growth: '2x in 3 months',
        required: ['client', 'server', 'database'],
        bonus: [{ component: 'cache', xp: 25, label: 'Add caching layer (+25 XP)' },{ component: 'monitoring', xp: 15, label: 'Include monitoring (+15 XP)' },{ component: 'loadbalancer', xp: 35, label: 'Implement auto-scaling (+35 XP)' }],
      }),
      components: mvpComponents,
      feedbackData: JSON.stringify({
        learned: ['Load balancers improve availability and distribute traffic','Caching layers significantly reduce database load','Proper architecture planning prevents costly redesigns'],
        nextMission: 'scaling-up', nextPreview: 'Your MVP attracted 50,000 users. Time to scale!',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: scalingComponents, learningPath: 'foundations', skillLevel: 'beginner' },
    create: {
      slug: 'scaling-up', title: 'Mission 2: Scaling Up',
      difficulty: 2, estimatedTime: '20-30 min', xpReward: 300, order: 2,
      learningPath: 'foundations', skillLevel: 'beginner',
      ...
      objectives: JSON.stringify(['Handle 10,000 concurrent users','Maintain 99.9% availability','Respond in under 150ms','Stay within $2,000/month budget','Introduce microservices architecture']),
      requirements: JSON.stringify({
        traffic: { concurrent: 10000, daily: 100000 },
        performance: { latencyMs: 150, availability: 99.9 },
        budget: 2000, growth: '5x in 6 months',
        required: ['client', 'loadbalancer', 'server', 'database', 'cache'],
        bonus: [{ component: 'queue', xp: 40, label: 'Add message queue (+40 XP)' },{ component: 'monitoring', xp: 30, label: 'Add monitoring (+30 XP)' },{ component: 'cdn', xp: 25, label: 'Add CDN (+25 XP)' }],
      }),
      components: scalingComponents,
      feedbackData: JSON.stringify({
        learned: ['Horizontal scaling handles traffic spikes effectively','Database replicas improve read performance','Message queues enable async processing'],
        nextMission: 'global-expansion', nextPreview: 'Series B secured! Time to go global across 3 continents.',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: globalComponents, learningPath: 'foundations', skillLevel: 'intermediate' },
    create: {
      slug: 'global-expansion', title: 'Mission 3: Global Expansion',
      difficulty: 3, estimatedTime: '30-45 min', xpReward: 500, order: 3,
      learningPath: 'foundations', skillLevel: 'intermediate',
      ...
      objectives: JSON.stringify(['Multi-region deployment across 3 continents','Maintain 99.99% availability','Achieve sub-100ms global response time','Optimize cost for global infrastructure','Implement disaster recovery']),
      requirements: JSON.stringify({
        traffic: { concurrent: 100000, daily: 1000000 },
        performance: { latencyMs: 100, availability: 99.99 },
        budget: 10000, growth: 'global',
        required: ['client', 'loadbalancer', 'server', 'database', 'cache', 'cdn'],
        bonus: [{ component: 'queue', xp: 50, label: 'Add global queue (+50 XP)' },{ component: 'storage', xp: 40, label: 'Add distributed storage (+40 XP)' },{ component: 'monitoring', xp: 35, label: 'Add global monitoring (+35 XP)' }],
      }),
      components: globalComponents,
      feedbackData: JSON.stringify({
        learned: ['CDN reduces latency for global users dramatically','Multi-region deployments improve availability and compliance','Distributed databases balance consistency and performance'],
        nextMission: 'file-converter', nextPreview: 'Global architect achieved! Now build a real-world file conversion service.',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: fileConverterComponents, learningPath: 'async-queues', skillLevel: 'intermediate' },
    create: {
      slug: 'file-converter', title: 'Mission 4: File Converter',
      difficulty: 2, estimatedTime: '20-25 min', xpReward: 400, order: 4,
      ...
      objectives: JSON.stringify(['Handle 5,000 concurrent upload requests','Maintain 99% availability','API response under 280ms','Stay within $800/month budget','Process conversions asynchronously via queues']),
      requirements: JSON.stringify({
        traffic: { concurrent: 5000, daily: 50000 },
        performance: { latencyMs: 280, availability: 99.0 },
        budget: 800, growth: '3x in 6 months',
        required: ['client', 'server', 'queue', 'storage', 'database'],
        bonus: [{ component: 'monitoring', xp: 25, label: 'Add monitoring (+25 XP)' },{ component: 'apigateway', xp: 35, label: 'Add API Gateway (+35 XP)' },{ component: 'loadbalancer', xp: 40, label: 'Add Load Balancer (+40 XP)' }],
      }),
      components: fileConverterComponents,
      feedbackData: JSON.stringify({
        learned: ['Message queues decouple upload ingestion from file processing workers','Object storage is ideal for large binary files — never store files in the DB','Async architecture returns fast API responses while jobs run in the background'],
        nextMission: 'code-judge', nextPreview: 'File converter shipped! Now build an isolated code execution engine.',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: codeJudgeComponents, learningPath: 'async-queues', skillLevel: 'intermediate' },
    create: {
      slug: 'code-judge', title: 'Mission 7: Code Judge',
      difficulty: 4, estimatedTime: '25-30 min', xpReward: 450, order: 7,
      learningPath: 'async-queues', skillLevel: 'intermediate',
      ...
      objectives: JSON.stringify(['Handle 2,000 concurrent code submissions','Maintain 99.5% availability','API response under 280ms (verdicts are async)','Stay within $2,500/month budget']),
      requirements: JSON.stringify({
        traffic: { concurrent: 2000, daily: 20000 },
        performance: { latencyMs: 280, availability: 99.5 },
        budget: 2500, growth: '5x during programming competitions',
        required: ['client', 'server', 'queue', 'storage', 'database'],
        bonus: [{ component: 'monitoring', xp: 35, label: 'Add monitoring (+35 XP)' },{ component: 'apigateway', xp: 30, label: 'Add API Gateway (+30 XP)' },{ component: 'loadbalancer', xp: 40, label: 'Add Load Balancer (+40 XP)' }],
      }),
      components: codeJudgeComponents,
      feedbackData: JSON.stringify({
        learned: ['Worker queue pattern isolates resource-heavy execution from the API response path','Object storage safely persists job inputs, outputs, and execution artifacts','Queue depth monitoring prevents submission backlogs during contest traffic spikes'],
        nextMission: null, nextPreview: 'Async & Queues path complete! Try the High-Read Systems path next.',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: urlShortenerComponents, learningPath: 'high-read', skillLevel: 'intermediate' },
    create: {
      slug: 'url-shortener', title: 'Mission 5: URL Shortener',
      difficulty: 3, estimatedTime: '20-25 min', xpReward: 500, order: 5,
      learningPath: 'high-read', skillLevel: 'intermediate',
      ...
      objectives: JSON.stringify(['Handle 50,000 concurrent redirect requests','Maintain 99.9% availability','Resolve URLs in under 170ms','Stay within $1,500/month budget']),
      requirements: JSON.stringify({
        traffic: { concurrent: 50000, daily: 500000 },
        performance: { latencyMs: 170, availability: 99.9 },
        budget: 1500, growth: '10x in 12 months',
        required: ['client', 'server', 'database', 'cache'],
        bonus: [{ component: 'cdn', xp: 40, label: 'Add CDN for landing pages (+40 XP)' },{ component: 'monitoring', xp: 25, label: 'Add monitoring (+25 XP)' },{ component: 'apigateway', xp: 30, label: 'Add API Gateway (+30 XP)' }],
      }),
      components: urlShortenerComponents,
      feedbackData: JSON.stringify({
        learned: ['Read-heavy systems benefit most from aggressive caching at every layer','Cache-aside pattern for URL lookups reduces database load by up to 99%','CDN edge nodes reduce redirect latency and origin server pressure globally'],
        nextMission: 'search-engine', nextPreview: '50,000 concurrent users handled. Now index millions of documents!',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: searchEngineComponents, learningPath: 'high-read', skillLevel: 'advanced' },
    create: {
      slug: 'search-engine', title: 'Mission 8: Search Engine',
      difficulty: 4, estimatedTime: '30-35 min', xpReward: 650, order: 8,
      ...
      objectives: JSON.stringify(['Handle 75,000 concurrent search queries','Maintain 99.9% availability','Return search results under 160ms','Stay within $5,000/month budget']),
      requirements: JSON.stringify({
        traffic: { concurrent: 75000, daily: 750000 },
        performance: { latencyMs: 160, availability: 99.9 },
        budget: 5000, growth: '20% per quarter',
        required: ['client', 'server', 'database', 'cache', 'storage'],
        bonus: [{ component: 'cdn', xp: 35, label: 'Add CDN for search UI (+35 XP)' },{ component: 'queue', xp: 40, label: 'Add async indexing queue (+40 XP)' },{ component: 'monitoring', xp: 25, label: 'Add monitoring (+25 XP)' }],
      }),
      components: searchEngineComponents,
      feedbackData: JSON.stringify({
        learned: ['Inverted index structures enable fast full-text search across millions of documents','Async indexing via queues decouples write-heavy indexing from read-heavy search','Caching top queries eliminates repeated expensive index traversals'],
        nextMission: null, nextPreview: 'High-Read Systems path complete! Try Consistency & Transactions next.',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: liveScoreboardComponents, learningPath: 'real-time', skillLevel: 'intermediate' },
    create: {
      slug: 'live-scoreboard', title: 'Mission 6: Live Scoreboard',
      difficulty: 3, estimatedTime: '25-30 min', xpReward: 600, order: 6,
      ...
      objectives: JSON.stringify(['Handle 80,000 concurrent viewers','Maintain 99.9% availability','Push score updates under 150ms','Stay within $3,000/month budget']),
      requirements: JSON.stringify({
        traffic: { concurrent: 80000, daily: 1000000 },
        performance: { latencyMs: 150, availability: 99.9 },
        budget: 3000, growth: 'spiky — 10x peaks during finals',
        required: ['client', 'server', 'queue', 'cache', 'database'],
        bonus: [{ component: 'cdn', xp: 40, label: 'Add CDN for static assets (+40 XP)' },{ component: 'apigateway', xp: 30, label: 'Add API Gateway (+30 XP)' },{ component: 'monitoring', xp: 25, label: 'Add monitoring (+25 XP)' }],
      }),
      components: liveScoreboardComponents,
      feedbackData: JSON.stringify({
        learned: ['Message queues act as pub/sub brokers to broadcast real-time score events','Cache is critical for read-heavy dashboards — database must never be polled per viewer','Horizontal scaling with multiple servers handles unpredictable traffic spikes'],
        nextMission: 'ride-hailing', nextPreview: '80,000 fans handled. Now match drivers to riders in real time!',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    update: { components: rideHailingComponents, learningPath: 'real-time', skillLevel: 'advanced' },
    create: {
      slug: 'ride-hailing', title: 'Mission 11: Ride Hailing',
      difficulty: 5, estimatedTime: '35-40 min', xpReward: 700, order: 11,
      ...
      objectives: JSON.stringify(['Handle 25,000 concurrent ride requests','Maintain 99.9% availability','Match drivers in under 155ms','Stay within $8,000/month budget']),
      requirements: JSON.stringify({
        traffic: { concurrent: 25000, daily: 250000 },
        performance: { latencyMs: 155, availability: 99.9 },
        budget: 8000, growth: '4x peak demand during rush hours',
        required: ['client', 'server', 'database', 'cache', 'queue'],
        bonus: [{ component: 'cdn', xp: 30, label: 'Add CDN for map tiles (+30 XP)' },{ component: 'apigateway', xp: 35, label: 'Add API Gateway (+35 XP)' },{ component: 'monitoring', xp: 30, label: 'Add monitoring (+30 XP)' }],
      }),
      components: rideHailingComponents,
      feedbackData: JSON.stringify({
        learned: ['In-memory geo-indexes in cache power sub-second driver proximity queries','Event-driven matching via queues scales elastically with demand spikes','CDN is essential for map tile delivery at global scale'],
        nextMission: null, nextPreview: 'Real-Time Systems path complete! Try Scale & Streaming next.',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    create: {
      ...
      objectives: JSON.stringify(['Handle 10,000 concurrent booking requests','Maintain 99.9% availability','Confirm bookings under 200ms','Stay within $3,500/month budget','Prevent double-booking race conditions']),
      requirements: JSON.stringify({
        traffic: { concurrent: 10000, daily: 100000 },
        performance: { latencyMs: 200, availability: 99.9 },
        budget: 3500, growth: 'seasonal spikes (3x during holidays)',
        required: ['client', 'server', 'database', 'cache', 'queue'],
        bonus: [{ component: 'apigateway', xp: 35, label: 'Add API Gateway (+35 XP)' },{ component: 'monitoring', xp: 30, label: 'Add monitoring (+30 XP)' },{ component: 'storage', xp: 20, label: 'Add storage for booking receipts (+20 XP)' }],
      }),
  ...
  await prisma.mission.upsert({
    ...
    update: { components: paymentComponents, learningPath: 'consistency', skillLevel: 'advanced' },
    create: {
      slug: 'payment-processing', title: 'Mission 13: Payment Processing',
      difficulty: 5, estimatedTime: '35-45 min', xpReward: 800, order: 13,
      learningPath: 'consistency', skillLevel: 'advanced',
      ...
      objectives: JSON.stringify(['Handle 5,000 concurrent payment requests','Maintain 99% availability','Process payments under 280ms','Stay within $5,000/month budget','Guarantee idempotent transaction processing']),
      requirements: JSON.stringify({
        traffic: { concurrent: 5000, daily: 50000 },
        performance: { latencyMs: 280, availability: 99.0 },
        budget: 5000, growth: 'doubles every quarter',
        required: ['client', 'server', 'database', 'queue', 'monitoring'],
        bonus: [{ component: 'apigateway', xp: 45, label: 'Add API Gateway for auth (+45 XP)' },{ component: 'loadbalancer', xp: 35, label: 'Add Load Balancer (+35 XP)' },{ component: 'cache', xp: 30, label: 'Add idempotency key cache (+30 XP)' }],
      }),
      components: paymentComponents,
      feedbackData: JSON.stringify({
        learned: ['Idempotency keys stored in a queue guarantee exactly-once payment processing','Monitoring and audit logs are regulatory requirements for all payment systems','ACID database transactions prevent partial or inconsistent payment states'],
        nextMission: null, nextPreview: 'Consistency & Transactions path complete! You are a System Design Master! 🏆',
      }),
    },
  });
  ...
  await prisma.mission.upsert({
    ...
    create: {
      ...
      objectives: JSON.stringify(['Handle 500,000 concurrent feed readers','Maintain 99.99% availability','Load timelines under 160ms','Stay within $15,000/month budget','Support fan-out delivery for posts to followers']),
      requirements: JSON.stringify({
        traffic: { concurrent: 500000, daily: 5000000 },
        performance: { latencyMs: 160, availability: 99.99 },
        budget: 15000, growth: 'viral — unpredictable 100x spikes',
        required: ['client', 'server', 'database', 'cache', 'queue'],
        bonus: [{ component: 'cdn', xp: 40, label: 'Add CDN for media delivery (+40 XP)' },{ component: 'storage', xp: 35, label: 'Add object storage for media (+35 XP)' },{ component: 'monitoring', xp: 25, label: 'Add monitoring (+25 XP)' }],
      }),
  ...
  await prisma.mission.upsert({
    ...
    create: {
      ...
      objectives: JSON.stringify(['Serve 500,000 concurrent video streams','Maintain 99.99% availability','Stream initiation under 160ms','Stay within $50,000/month budget']),
      requirements: JSON.stringify({
        traffic: { concurrent: 500000, daily: 5000000 },
        performance: { latencyMs: 160, availability: 99.99 },
        budget: 50000, growth: 'catalog grows 10% monthly',
        required: ['client', 'server', 'cdn', 'storage', 'database'],
        bonus: [{ component: 'queue', xp: 45, label: 'Add encoding queue (+45 XP)' },{ component: 'cache', xp: 35, label: 'Add metadata cache (+35 XP)' },{ component: 'monitoring', xp: 25, label: 'Add monitoring (+25 XP)' }],
      }),
