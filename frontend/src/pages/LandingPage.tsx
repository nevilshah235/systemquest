...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white font-sans">
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                onClick={handleCTA}
                className="px-8 py-3.5 bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-xl transition-all hover:shadow-lg hover:shadow-violet-500/25 active:scale-95"
              >
                Start Free — No Credit Card
              </button>
              <a
                href="#how-it-works"
                className="px-8 py-3.5 border border-white/10 hover:border-violet-500/50 text-gray-300 hover:text-white font-semibold rounded-xl transition-all text-center"
              >
                See How It Works
              </a>
            </div>
      ...
      {/* ── Problem Statement ───────────────────────────────────────────── */}
      <section className="py-16 px-6 md:px-12 lg:px-24 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <p className="text-center text-gray-500 text-sm font-semibold uppercase tracking-widest mb-10">
            System design interviews are hard. Textbooks are boring.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {[
              { icon: '📚', label: 'Outdated Info' },
              { icon: '💤', label: 'Dedicated Info' },
              { icon: '⏰', label: 'No Time to Practice' },
              { icon: '🤷', label: 'No Guidance' },
              { icon: '🌫️', label: 'Lack of Clarity' },
              { icon: '😕', label: 'Lack of Clarity' },
            ].map((p, i) => (
              <div key={i} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-gray-900/50 border border-white/5">
                <span className="text-2xl">{p.icon}</span>
                <span className="text-gray-500 text-xs text-center">{p.label}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="flex flex-col gap-6">
              {[
                ...
                <div key={item.step} className="flex gap-4 p-4 rounded-xl bg-gray-900/60 border border-white/5 hover:border-violet-500/30 transition-all">
                  <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-violet-600/20 border border-violet-500/30 flex items-center justify-center text-lg">
                    {item.icon}
                  </div>
                  <div>
      ...
      <section id="leaderboard" className="py-20 px-6 md:px-12 lg:px-24">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-start">
          {/* Leaderboard */}
          <div>
            <h2 className="text-2xl font-bold mb-8 text-white">LEADERBOARD</h2>
            <div className="rounded-2xl border border-white/10 bg-gray-900/60 overflow-hidden">
              <div className="px-4 py-3 border-b border-white/5 flex justify-between text-xs text-gray-500 font-semibold uppercase">
                <span>Rank</span><span>Engineer</span><span>XP</span>
              </div>
              {[
                { rank: 1, name: 'alexdev', xp: '24,800', badge: '🥇' },
                { rank: 2, name: 'sysdesign_pro', xp: '22,150', badge: '🥈' },
                { rank: 3, name: 'infra_ninja', xp: '19,600', badge: '🥉' },
                { rank: 4, name: 'cloudarchitect', xp: '17,200', badge: '⭐' },
                { rank: 5, name: 'bytewrangler', xp: '15,900', badge: '⭐' },
              ].map((entry) => (
                <div key={entry.rank} className="flex items-center justify-between px-4 py-3 border-b border-white/5 hover:bg-violet-500/5 transition-colors">
                  <span className="text-lg w-8">{entry.badge}</span>
                  ...
                  <span className="text-amber-400 font-bold text-sm font-mono">{entry.xp}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-8">
            <div>
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  <div key={i} className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-900/60 border border-white/5 hover:border-violet-500/30 transition-all">
                    <span className="text-2xl">{b.icon}</span>
                    <span className="text-gray-500 text-[10px] text-center">{b.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { value: users.toLocaleString() + '+', label: 'Users', suffix: '' },
                { value: missions + '+', label: 'Missions\nCompleted', suffix: '' },
                { value: missionCount + '+', label: 'Missions', suffix: '' },
              ].map((s, i) => (
                <div key={i} className="text-center p-4 rounded-xl bg-gray-900/60 border border-white/5">
                  <div className="text-2xl font-extrabold text-white">{s.value}</div>
                  <div className="text-gray-500 text-xs whitespace-pre-line mt-1">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            {[
              {
                ...
                subtitle: 'Design MVP architectures',
                desc: 'Build lean, scalable systems. From single-server MVPs to multi-region deployments. Perfect for engineers building their own products.',
                missions: 12,
                tag: 'Free',
              ...
              {
                icon: '📈',
                title: 'Scale-Up Engineer',
                subtitle: 'Build distributed systems',
                desc: 'Tackle high-traffic challenges. Design systems that handle millions of users with high availability and minimal latency.',
                missions: 18,
                tag: 'Pro',
              ...
              },
              {
                ...
                subtitle: 'Ace FAANG interviews',
                desc: 'Real interview questions from top tech companies. Practice URL shorteners, news feeds, payment systems, and more.',
                missions: 24,
                tag: 'Pro',
              ...
              <div
                key={s.title}
                className={`relative rounded-2xl bg-gradient-to-br ${s.gradient} border ${s.border} p-6 hover:scale-[1.02] transition-transform cursor-pointer`}
              >
            ...
            {[
              {
                ...
                features: [
                  '1 Storyline (Startup Founder)',
                  'First 3 missions unlocked',
              ...
              },
              {
                ...
                features: [
                  'All 3 storylines unlocked',
                  '50+ missions & counting',
              ...
              <div
                ...
                className={`rounded-2xl p-8 border ${
                  plan.primary
                    ? 'bg-gradient-to-br from-violet-600/20 to-blue-600/10 border-violet-500/40'
                    : 'bg-gray-900/60 border-white/10'
                }`}
              >
                {plan.primary && (
                  <span className="inline-block text-xs font-semibold bg-violet-500 text-white px-3 py-1 rounded-full mb-4">
                    Most Popular
                  </span>
                )}
                <div className="mb-6">
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-xl font-semibold transition-all ${
                    plan.primary
                      ...
                      : 'border border-white/10 hover:border-violet-500/50 text-gray-300 hover:text-white'
                  }`}
                >
                  {plan.cta}
                </button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 mb-12">
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-white font-bold">SystemQuest</span>
              </div>
              <p className="text-gray-600 text-xs leading-relaxed">Master system design through gamified missions.</p>
            </div>
            {[
              { heading: 'Platform', links: ['Dashboard', 'Missions', 'Leaderboard'] },
              { heading: 'Features', links: ['Builder', 'Simulator', 'Feedback'] },
              { heading: 'Resources', links: ['Blog', 'Threads', 'Community'] },
              { heading: 'Company', links: ['About Us', 'Contact', 'Careers'] },
            ].map((col) => (
              <div key={col.heading}>
                <h4 className="text-white text-xs font-semibold uppercase tracking-wider mb-3">{col.heading}</h4>
                <ul className="flex flex-col gap-2">
                  {col.links.map((l) => (
                    <li key={l}><a href="#" className="text-gray-600 hover:text-gray-300 text-xs transition-colors">{l}</a></li>
                  ))}
                </ul>
              </div>
            ))}
