...
import { Architecture, ArchitectureComponent, Connection, ComponentType, getComponentMeta, migrateComponentType } from '../data/types';
...
export const useBuilderStore = create<BuilderState>()((set, get) => ({
  ...
  addComponent: (type, x, y) =>
    set((s) => {
      ...
      const label = `Added ${getComponentMeta(type).label}`;
  ...
  loadArchitecture: (arch) => {
    const migrated: Architecture = {
      components: arch.components.map((c) => ({ ...c, type: migrateComponentType(c.type) })),
      connections: arch.connections,
    };
    set({ architecture: migrated, isDirty: false, past: [], future: [], lastActionLabel: '' });
  },
